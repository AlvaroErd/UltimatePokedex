package com.alerdoci.ultimatepokedex.domain.utilities
...
fun Date?.format(format: String = "dd-MM-yyyy"): String =
    (this != null).iif({ SimpleDateFormat(format, Locale.getDefault()).format(this!!) }, { "" })

fun Instant?.format(format: String = "dd-MM-yyyy"): String = (this != null).iif({
    DateTimeFormatter.ofPattern(format, Locale.getDefault()).format(this@format)
}, { "" })

fun CharSequence?.toDate(format: String = "dd-MM-yyyy"): Date? = this.toString().toDate(format)
fun CharSequence?.toInstant(format: String = "dd-MM-yyyy"): Instant? =
    this.toString().toInstant(format)
...
fun String?.toInstant(format: String = "dd-MM-yyyy"): Instant? = try {
    this?.let {
        LocalDateTime.parse(it, DateTimeFormatter.ofPattern(format, Locale.getDefault())).toInstant(
            ZoneOffset.UTC
        )
    }
} catch (ex: Exception) {
...
fun Instant.getMondayOfWeek(): Instant = this.plus(
    (DayOfWeek.MONDAY.value - this.atZone(ZoneOffset.UTC).dayOfWeek.value).toLong(),
    ChronoUnit.DAYS
)
...
fun Instant.getFirstDayOfMonth(): Instant =
    this.atZone(ZoneOffset.UTC).withDayOfMonth(1).toInstant()